...
public final class EventService {
    ...
    public func outrights(forCompetition competitionID: Competition.ID, sport sportID: Sport.ID,
                          locale: Locale = .current) async throws -> [Event] {
        logger.debug("Fetching Outrights", metadata: ["competition-id": .stringConvertible(competitionID)])

        let result = try await scan.search(.outrights(forCompetition: competitionID, sport: sportID, locale: locale))
        guard let eventAttachments = result.attachments.events?.values else {
            return []
        }
